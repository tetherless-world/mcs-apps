...
trait KgStoreBehaviors extends Matchers { this: WordSpec =>
  def store(sut: KgStore) {
    "get edges by object" in {
      for (node <- TestCskgCsvData.nodes) {
    ...
    "page edges by object" in {
      ...
      val expected = TestCskgCsvData.edges.filter(edge => edge.`object` == node.id).sortBy(edge => (edge.subject, edge.predicate))
    ...
    "page edges by subject" in {
      val node = TestCskgCsvData.nodes(0)
      val expected = TestCskgCsvData.edges.filter(edge => edge.subject == node.id).sortBy(edge => (edge.predicate, edge.`object`))
    ...
    "get matching nodes by label" in {
      val expected = TestCskgCsvData.nodes(0)
    ...
    "get matching nodes count with no text search and no filters" in {
      sut.getMatchingNodesCount(filters = None, text = None) should equal(TestCskgCsvData.nodes.size)
    }

    "get matching nodes count with no text search but with filters" in {
      sut.getMatchingNodesCount(filters = Some(KgNodeFilters(sources = Some(StringFilter(exclude = None, include = Some(List(TestCskgCsvData.nodes(0).sources(0))))))), text = None) should equal(TestCskgCsvData.nodes.size)
      sut.getMatchingNodesCount(filters = Some(KgNodeFilters(sources = Some(StringFilter(exclude = Some(List(TestCskgCsvData.nodes(0).sources(0))))))), text = None) should equal(0)
    }
    ...
    "get total edges count" in {
      val expected = TestCskgCsvData.edges.size
      val actual = sut.getTotalEdgesCount
      actual should equal(expected)
    }

    "get total nodes count" in {
      val expected = TestCskgCsvData.nodes.size
      val actual = sut.getTotalNodesCount
      actual should equal(expected)
    }

    "get sources" in {
      val expected = TestCskgCsvData.nodes.flatMap(_.sources).toSet
    ...
    "filter out matching nodes" in {
      ...
      val actualCount = sut.getMatchingNodesCount(
        filters = Some(KgNodeFilters(sources = Some(StringFilter(exclude = Some(List(TestCskgCsvData.nodes(0).sources(0))), include = None)))),
        text = Some("Test")
      )
    ...
    "get a path by id" in {
      val expected = TestCskgCsvData.paths(0)
