...
object KgNodeSort {
  def sort(nodes: List[KgNode], sorts: List[KgNodeSort]): List[KgNode] = {
    ...
    else KgNodeSort.sort(nodes.sortBy(node => KgNodeField.getFieldString(node, sorts.last.field))(if (sorts.last.direction == SortDirection.Ascending) Ordering[String] else Ordering[String].reverse), sorts.init)
  }
}
