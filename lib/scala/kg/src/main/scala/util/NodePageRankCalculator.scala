...
class NodePageRankCalculator(val nodes: List[KgNode], val edges: List[KgEdge]) {
...
object NodePageRankCalculator {
  def apply(
    ...
    new NodePageRankCalculator(nodes, edges).calcNodePageRanks(
