...
abstract class AbstractPostgresKgStore(protected val dbConfigProvider: DatabaseConfigProvider) extends HasDatabaseConfigProvider[PostgresProfile] {
  ...
  private class KgEdgeTable(tag: Tag) extends Table[(String, String, String, String, String)](tag, "edge") {
  ...
  private class KgNodeTable(tag: Tag) extends Table[(String, Option[Short], Option[Short], Option[Float], Option[Char], Option[Short])](tag, "node") {
    def id = column[String]("id", O.PrimaryKey)
