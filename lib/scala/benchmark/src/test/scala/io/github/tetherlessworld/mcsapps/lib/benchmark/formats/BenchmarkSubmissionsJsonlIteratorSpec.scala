...
class BenchmarkSubmissionsJsonlIteratorSpec extends WordSpec with Matchers with WithResource {
  "Benchmark submissions .jsonl reader" can {
    "read the test data" in {
      withResource(BenchmarkSubmissionsJsonlIterator.open(TestBenchmarkDataResources.benchmarkSubmissionsJsonl.getAsStream())) { iterator =>
